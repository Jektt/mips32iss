...
__start:
	...
	addi		$t3, $t3, %lo(mem03)
	lw		$t1, 4($t3)
	TEST_EXPECT	$t1, 0x34354037

	# Zero offset, base not word aligned

	lui		$t0, %hi(mem04+7)
	addi		$t0, $t0, %lo(mem04+7)
	sb		$t2, 0($t0)
	lui		$t3, %hi(mem04)
	addi		$t3, $t3, %lo(mem04)
	lw		$t1, 4($t3)
	TEST_EXPECT	$t1, 0x34353640

	# Positive offset, base not word aligned

	lui		$t0, %hi(mem05+7)
	addi		$t0, $t0, %lo(mem05+7)
	sb		$t2, 2($t0)
	lui		$t3, %hi(mem05)
	addi		$t3, $t3, %lo(mem05)
	lw		$t1, 8($t3)
	TEST_EXPECT	$t1, 0x38403a3b

	# Negative offset, base not word aligned

	lui		$t0, %hi(mem06+7)
	addi		$t0, $t0, %lo(mem06+7)
	sb		$t2, -2($t0)
	lui		$t3, %hi(mem06)
	addi		$t3, $t3, %lo(mem06)
	lw		$t1, 4($t3)
	TEST_EXPECT	$t1, 0x34403637

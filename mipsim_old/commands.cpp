...
bool command_match_reg(string& command, unsigned int i, bool& data_present, unsigned int& num, uint32_t& data) {
  data_present = false;
...
void interpret_commands(memory* main_memory, processor* cpu, bool verbose) {
  ...
  while (true) {
    ...
    else if (command_match_reg(command, i, data_present, num, data)) {  // Check for $ command
      if (num > 31) {

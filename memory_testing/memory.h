...
class memory: public memory_accessible {
 ...
 private:
  list<Block> m_list;				// list container (doubly linked) of the 4096KB memory blocks structs that are allocated as required
  bool m_verbose;
 ...
 public:
  ...
  // Read a word of data from an address							@@@COMPLETE
  ...
  // Display on cout whether the address is present, and if so, display the data		@@@COMPLETE
  void probe_address (uint32_t address);

  // Report access stats (not for UG?)
  void report_accesses ();

  // Display the word of data at an address							@@@COMPLETE
  void show_address (uint32_t address);

  // Set the word of data at an address								@@@COMPLETE
  ...
  // @@@DEBUG display first 20 words of each allocated memory block
  void disp_mem();
